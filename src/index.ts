...
import { ResolvePluginInstance, Resolver, Tapable, TapAsyncCallback, TapAsyncInnerCallback, CreateInnerContext } from "./types";
...
export class NXTResolverPlugin implements ResolvePluginInstance {
  ...
  apply(resolver: Resolver): void {
    if (!resolver) {
      this.log.warn(
        "Found no resolver, not applying NXT-resolver"
      );
      return;
    }
